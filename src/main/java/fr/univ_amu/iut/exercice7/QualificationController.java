...
public class QualificationController {
  ...
  private void initialize() {
    ...
    colHorodatage.setCellValueFactory(
        data -> new SimpleStringProperty(HEURE.format(data.getValue().getHorodatage())));

    colFrequence.setCellValueFactory(
        data ->
            new SimpleStringProperty(
                String.format("%.1f kHz", data.getValue().getFrequenceDominanteKHz())));

    colDuree.setCellValueFactory(
        data -> new SimpleStringProperty(data.getValue().getDureeSecondes() + " s"));
